...
const ProjectItem = ({ img , title, dev, hv}) => {
  return (
    <div className='relative flex items-center justify-center h-auto w-full shadow-xl shadow-indigo-400 rounded-xl group hover:bg-gradient-to-r from-indigo-200 to-indigo-900'>
      ...
      <div className='hidden group-hover:block absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]'>
        ...
        <p className='pb-4 pt-2 text-white text-center'>{dev}</p>
        <a href={hv}>
          <p className='text-center p-3 rounded-lg bg-white text-indigo-800 font-bold cursor-pointer text-lg'>More Info</p>
        </a>    
      </div>
    </div>
  )
